...
- **T0.2** docker-compose для dev: Postgres + Redis + MinIO
  - **DoD**: сервисы стартуют, Django подключается к Postgres, может писать в MinIO (пока хотя бы тестом).
...
- **T6.2** Генерация изображений + сохранение в S3/MinIO
...
> **Делай `<TASK_ID>` по `docs/ROADMAP.md`, соблюдай `docs/DECISIONS.md` и концепции `docs/01–07`, tests-first где уместно, не расширяй scope, обновляй документацию только при необходимости, по итогам дай команды проверки и краткий итог.**
